# {{root_dir}} — universal project inbox

This folder is the canonical home of every deferred decision, design inconsistency, convention violation, improvement opportunity, missing test, open question, and other action item that emerges during design or implementation work in this project. It is managed by the `update-todos` skill.
...
This folder is the deferred-decision surface for any project that wants to track open questions, deferred refactors, and ambiguities without losing them between sessions. Every observation that would otherwise be lost — between session boundaries, between engineers, between a quick thought mid-refactor and the moment you had bandwidth to act on it — is captured here atomically, with enough context that a future engineer can rehydrate without archaeology.
...
| GTD forced-triage | Nothing is someday without an expiry | `expires` (default `default_expiry_days`, default 90); `review` refuses to proceed past expired items without decision. |
...
See `inbox-entry.md.tpl` and `active-entry.md.tpl` for the two phase-specific schemas.
...
- `diary-update` — add to the developer-diary.
- `brainstorm` — needs ideation.
- `refactor` — code quality; no behavior change.
- `investigate` — research spike.
- `test` — add or fix test coverage.
- `decide` — needs product-owner / human decision.
- `reproduce` — bug-flavored; needs repro.

`review` is a **mode** of the skill, not a next-step. `documentation` is subsumed by `spec-update` or `diary-update` depending on whether the change belongs in the project's spec or the engineering narrative.

## Boundary with the developer-diary

**developer-diary** = time-indexed narrative. Answers: "how did we get here, what did we try, what did we learn?"

**TODOs** (this folder) = action-indexed pointers. Answers: "what remains to be done, why, and by whom?"
...
/update-todos import --from <path>   # one-shot migration from a flat markdown bullet list
```
