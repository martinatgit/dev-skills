#!/usr/bin/env python3
"""Interactive configuration installer for the create-tutorial skill.

Two-scope writer:
    --scope user    -> writes ~/.config/create-tutorial/config.yaml
    --scope project -> writes <project_root>/.create-tutorial/config.yaml (default
                       when path-typed keys are involved)

Path-typed keys (tutorials_dir) are project-only by design: the tutorials
directory is a per-project artefact and must not bleed across projects when
...
SKILL_NAME = "create-tutorial"
ENV_PREFIX = "CREATE_TUTORIAL_"
...
PROMPT_SUGGESTIONS = {
    "tutorials_dir": "doc/tutorials",
}

PROMPTS = {
    "tutorials_dir": "Tutorials directory (relative to project root, or absolute)",
}

# Path-typed keys: refused at the user-config layer.
PATH_KEYS = {"tutorials_dir"}
...
def main():
    ...
    if args.scope == "project":
        ...
        if proot is None:
            print(
                ...
                "Run from inside a project. tutorials_dir is project-bound by\n"
    ...
    if (
        ...
        and not existing.get("tutorials_dir")
        and not getattr(args, "tutorials_dir", None)
    ):
        print("\ncreate-tutorial needs a tutorials directory.")
        ...
        print("  2. Configure just create-tutorial (creates .create-tutorial/config.yaml)")
        try:
        ...
        if choice == "1":
            ...
            try:
                ...
                print("\nWrote %s." % shared_path)
                print("create-tutorial will resolve tutorials_dir to %s/tutorials"
                      % docs_root)
                return 0
            except RuntimeError as e:
                print("error: %s" % e, file=sys.stderr)
                return 2
        # Choice 2 (or any other input): fall through to per-skill prompt loop below.
