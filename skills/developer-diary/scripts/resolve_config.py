#!/usr/bin/env python3
"""Resolve configuration values for the developer-diary skill.

Resolution order (first match wins):
    1. Environment variable DEVELOPER_DIARY_<UPPERCASE_KEY>
    2. <project_root>/.developer-diary/config.yaml
...
def resolve_all() -> dict[str, str]:
    ...
    # Derive feature_routing_file when blank.
    if not resolved.get("feature_routing_file") and resolved.get("root_dir"):
        resolved["feature_routing_file"] = str(
            Path(resolved["root_dir"]) / "feature-routing.md"
        )
    return resolved
